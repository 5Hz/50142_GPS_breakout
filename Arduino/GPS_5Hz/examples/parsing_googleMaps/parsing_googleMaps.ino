...
// e imprime en el monitor serial la información analizada,
// además de un link a google maps para visualizar tu posición
//
...
void loop()                     // run over and over again
{
  ...
  if (millis() - timer > 2000) { 
    ...
    //Imprime la información relevante a tu posición solo cuando el GPS ha hecho fix
    if (GPS.fix) {
      Serial.println();
      Serial.println("--*--*--*--*--*--*--*--*--*--*--*--");
      //El caracter 176 ASCII es el simbolo de grados
      Serial.print("latitud: "); Serial.print(GPS.getDegrees(LATITUDE)); Serial.print((char)176); Serial.print(GPS.getMinutes(LATITUDE), 4); Serial.println("'");
      Serial.print("longitud: "); Serial.print(GPS.getDegrees(LONGITUDE)); Serial.print((char)176); Serial.print(GPS.getMinutes(LONGITUDE), 4); Serial.println("'");
      Serial.print("Location (minutes): ");
      GPS.setNotation(MINUTES);
      Serial.print(GPS.getLatitude(), 4); Serial.print(GPS.lat);
      Serial.print(", "); 
      Serial.print(GPS.getLongitude(), 4); Serial.println(GPS.lon);
      Serial.print("Location (decimals): ");
      GPS.setNotation(DECIMALS);
      Serial.print(GPS.getLatitude(), 4); Serial.print(GPS.lat);
      Serial.print(", "); 
      Serial.print(GPS.getLongitude(), 4); Serial.println(GPS.lon);
      
      Serial.print("Google Maps: ");
      GPS.printURL(); //Imprime URL para ver ubicacion en google maps
